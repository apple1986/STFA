...
class SynapseDataset(Dataset):
    def __init__(self, name, root, mode, size=None, id_path=None, nsample=None):
        ...
        if mode == 'train_l' or mode == 'train_u':
            ...
            with open('/home/data/lxs/SSL1/SSL4MIS-master/SSL4MIS-master/splits/%s/val.txt' % name, 'r') as f:
                self.ids = f.read().splitlines()

    def __getitem__(self, item):
        id = self.ids[item]
        if self.mode == "train_l" or self.mode  == "train_u":
            sample = np.load(os.path.join(self.root+"/data/train_npz", id+".npz"), 'r')
        else:
            sample = h5py.File(os.path.join(self.root+"/data/val", id+".npy.h5"), 'r')
        img = sample['image'][:]
        mask = sample['label'][:]
