...
def dequeue_and_enqueue(keys, queue, queue_ptr, queue_size):
    ...
    if queue[0].shape[0] == 0:
        # 如果队列为空，可以直接将所有 keys 放入队列
        queue[0] = keys.cpu()
        ptr = batch_size
    else:
        # 如果队列不为空，需要确保放入队列的 keys 大小不超过规定的大小
        if queue[0].shape[0] + batch_size > queue_size:
            # 如果队列大小将要超过规定大小，只取前面一部分 keys 放入队列
            remaining_space = queue_size - queue[0].shape[0]
            queue[0] = torch.cat((queue[0], keys[:remaining_space, :].cpu()), dim=0)
            ptr = queue_size
        else:
            # 如果队列大小不超过规定大小，将所有 keys 放入队列
