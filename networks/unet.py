...
from networks.cloformer import EfficientAttention
import random
...
class Encoder1(nn.Module):
    def __init__(self, params):
        ...
        self.dropout1 = self.params['dropout1']
        assert (len(self.ft_chns) == 5)
        self.in_conv = ConvBlock(
            self.in_chns, self.ft_chns[0], self.dropout[0])
        # self.in_conv1 = ConvBlock(
        #     self.in_chns*4, self.ft_chns[0], self.dropout[0])
        self.part1 = Partial_conv3(self.ft_chns[0] ,2,'split_cat')
        self.part2 = Partial_conv3(self.ft_chns[1] ,2,'split_cat')
        self.part3 = Partial_conv3(self.ft_chns[2] ,2,'split_cat')
        self.part4 = Partial_conv3(self.ft_chns[3] ,2,'split_cat')
    ...
    def forward(self, x):
        ...
        #     x0 = self.in_conv(x)
        # else:
        x0 = self.part1(self.in_conv(x))
        
        x1 = self.part2(self.down1(x0))
        x2 = self.part3(self.down2(x1))
        x3 = self.part4(self.down3(x2))
        x4 = self.down4(x3)
...
class Decoder_fft(nn.Module):
    def __init__(self, params):
        super(Decoder_fft, self).__init__()
        self.params = params
        self.in_chns = self.params['in_chns']
        self.ft_chns = self.params['feature_chns']
        self.n_class = self.params['class_num']
...
class UNet(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        #self.g = ConvGuidedFilter()
    def forward(self, x):
        #x = self.g(x,x,x)
        feature = self.encoder(x)
        output = self.decoder(feature)
        if output.shape[2] != 256 and output.shape[2] != 224:
...
class FFParser(nn.Module):
    def __init__(self, dim, h=56, w=56):
        ...
        self.complex_weight1 = nn.Parameter(torch.randn(dim, h, w, 2, dtype=torch.float32) * 0.02)
        self.complex_weight2 = nn.Parameter(torch.randn(dim, h, w, 2, dtype=torch.float32) * 0.02)
        self.w = w
        self.h = h
    def forward(self, x, spatial_size=None):
        ...
        if spatial_size is None:
            ...
            a, b = spatial_size
        x = np.fft.fftn(x.detach().cpu().numpy(),axes = (-2,-1))
        #x = torch.fft.fft2(x, dim=(2, 3), norm='ortho')
        weight = torch.view_as_complex(self.complex_weight).detach().cpu().numpy()
        x = x * weight
        x = np.fft.ifftshift(x)
        x =  torch.tensor(np.abs(x)).cuda()
        #x = torch.fft.ifft2(x, s=(a, b), dim=(2, 3), norm='ortho')

        x = x.reshape(B, C, H, W).to(torch.float32)
...
class UNet_fft(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.encoder = Encoder(params)
        self.decoder = Decoder(params)
        self.decoder_fft = Decoder_fft(params)
        self.fft = FFParser(256,16,16)
    def forward(self, x):
        feature = self.encoder(x)
        feature_fft = [f.clone() for f in feature]
        feature_fft[-1] = self.fft(feature[-1])
        output = self.decoder(feature)
        output_fft = self.decoder_fft(feature_fft)
        return output,output_fft
...
class UNet_trans(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.encoder = Encoder1(params)
        self.ema = EMA(16)
        self.ema1 = EMA(32)
        self.ema2 = EMA(64)
        self.ema3 = EMA(128)
        self.ema4 = EMA(256)
        self.decoder = Decoder1(params)

    def forward(self, x):
        feature = self.encoder(x)
        
        x0,x1,x2,x3,x4 = feature[0],feature[1],feature[2],feature[3],feature[4]
        feature[0] = self.ema(feature[0]) + x0
        feature[1] = self.ema1(feature[1]) + x1
        feature[2] = self.ema2(feature[2]) + x2
        feature[3] = self.ema3(feature[3]) + x3
        feature[4] = self.ema4(feature[4]) + x4
